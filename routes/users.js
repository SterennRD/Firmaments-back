            ...
            '$addFields': {
                'reading_lists.stories.total_fav': {
                    '$cond': [
                        {
                            '$gte': [
                                '$reading_lists.stories._id', null
                            ]
                        }, {
                            '$filter': {
                                'input': {
                ...
                'reading_lists.stories.faved': {
                    '$cond': [
                        {
                            '$eq': [
                                '$reading_lists.stories._id', null
                            ]
                        }, '$reading_lists.stories.faved', '$$REMOVE'
                    ]
                }
            }
        }, {
...
router.post('/reading-lists/details/:idRL/edit', (req, res) => {
    ...
    User.findOneAndUpdate(query, {$set:{"reading_lists.$.title": data.title, "reading_lists.$.description": data.description, "reading_lists.$.private": data.private }}, {new: true}, async function (err, user) {
        if (err) return err
        let readingList = await User.aggregate(queryRL).exec();
        res.json(readingList[0])
    });
...
router.post('/add/readinglist/:id/:idStory', VerifyToken, function(req, res, next) {
    ...
    const queryUser = [
        {
            '$match': {
                'reading_lists._id': new ObjectId(id)
            }
        }, {
            '$project': {
                'reading_lists': {
    ...
    User.findOneAndUpdate(query,{$pull:{"reading_lists.$.stories":new ObjectId(idStory)}}, {new: true}, async function (err, user) {
        ...
        if (!user) {
            User.findOneAndUpdate({"reading_lists":{"$elemMatch":{_id:new ObjectId(id)}}} ,{$push:{"reading_lists.$.stories":new ObjectId(idStory)}}, {new: true}, async function (err, user) {
                ...
                let readingList = await User.aggregate(queryUser).exec();
                res.json({user: readingList[0], story: story[0]})
            });
...
router.post('/new/readinglist/:id/:idStory', VerifyToken, function(req, res, next) {
    ...
    User.findOneAndUpdate({_id: new ObjectId(id)}, { $push: { "reading_lists": reading_list } }, {new: true}, async function (err, user) {
        if (err) return err
        let idRL = user.reading_lists.pop();
        idRL = idRL._id;
        const query = [
            {
                '$match': {
        ...
        let rl = await User.aggregate(query).exec();
